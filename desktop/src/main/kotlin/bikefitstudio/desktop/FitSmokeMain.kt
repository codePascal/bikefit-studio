 ...
 * orbits the bottom bracket so the crank sweeps through TDC/3-o'clock/BDC, driving cycle detection,
 * aggregation, and the FitEngine — no GUI, no ML.
 */
fun main() {
    val bbX = 0.5f
    val bbY = 0.6f
    ...
    repeat(6) { // revolutions
        for (deg in 0 until 360 step 10) {
            ...
            val lms =
                MutableList(PoseLandmarkIndex.LANDMARK_COUNT) { Landmark(0.5f, 0.5f, 0f, 1f, 1f) }
            ...
            lms[PoseLandmarkIndex.LEFT_KNEE] =
                Landmark((0.45f + fx) / 2f + 0.05f, (0.45f + fy) / 2f, 0f, 1f, 1f)
    ...
    println(
        "[fit] cycles=${summary.cycleCount}  grade=${summary.grade}  issues=${summary.totalIssueCount}"
    )
    if (summary.recommendations.isEmpty()) {
        println("[fit] (no recommendations)")
    } else {
        summary.recommendations.forEach {
            println("[fit]  - [${it.severity}] ${it.title} -> ${it.action}")
        }
    }
    println("[fit] done.")
}
