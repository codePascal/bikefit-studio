 ...
 *
 * Per frame it records joint angles, then edge-triggers on crank position (TDC/BDC/3 o'clock)
 * to delimit pedal cycles, record KOPS, and feed the [CycleAggregator]. [finish] runs the
 * [FitEngine] over the aggregated cycles.
 *
...
class BikeFitAnalyzer(
    ...
    val cycleCount: Int get() = aggregator.getCycleCount()

    fun process(frame: PoseFrame) {
        val knee = KneeAngleCalculator.calculateKneeAngleFromLandmarks(frame.landmarks, side)
            .takeIf { it.isValid }?.angle
        val hip = HipAngleCalculator.calculateHipAngleFromLandmarks(frame.landmarks, side)
            .takeIf { it.isValid }?.angle
        val ankle = AnkleAngleCalculator.calculateAnkleAngleFromLandmarks(frame.landmarks, side)
            .takeIf { it.isValid }?.angle
        val torsoResult = TorsoAngleCalculator.calculateTorsoAngleFromLandmarks(
            frame.landmarks, side, imageWidth = frame.imageWidth, imageHeight = frame.imageHeight
        )
        val torso = torsoResult.takeIf { it.isValid }?.angle

        aggregator.addMeasurement(
            frame.frameNumber, frame.timestampMs,
            kneeAngle = knee, hipAngle = hip, torsoAngle = torso, ankleAngle = ankle
        )

        val crankAngle = crankAngleDegrees(frame)
        val event = crankAngle?.let {
        ...
        when (event.position) {
            CrankPosition.BDC -> aggregator.endCycleAtBdc(
                frame.frameNumber, frame.timestampMs, kneeAngle = knee, ankleAngle = ankle
            )
    ...
    private fun crankAngleDegrees(frame: PoseFrame): Float? {
        ...
        var deg = Math.toDegrees(atan2((foot.y - bb.y).toDouble(), (foot.x - bb.x).toDouble())).toFloat()
        if (deg < 0f) deg += 360f
        return deg
    }
}
