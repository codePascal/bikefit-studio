...
/**
 * Headless end-to-end check of the pose sidecar: spawns the Python server, sends one frame,
 * and (if a pose is returned) runs it through the reused KOPS biomechanics. No GUI required.
 *
 * args: [scriptPath] [pythonExe] [imagePath]
 * If imagePath is given it is sent to the server; otherwise a blank frame is used.
 */
fun main(args: Array<String>) {
    ...
    val image: BufferedImage = imagePath
        ?.let { ImageIO.read(File(it)) ?: error("could not read image: $it") }
        ?.also { println("[smoke] image='$imagePath' (${it.width}x${it.height})") }
        ?: BufferedImage(640, 480, BufferedImage.TYPE_INT_RGB).also { println("[smoke] image=blank 640x480") }
    ...
    try {
        ...
        if (frame == null) {
            ...
            println("[smoke] detect -> ${frame.landmarks.size} landmarks, confidence=${"%.3f".format(frame.confidence)}")
            ...
            val kops = KneeOverPedalOffset.computeAtFrame(frame, BodySide.LEFT, calibration, crankScale = 0.1f)
            println("[smoke] KOPS -> valid=${kops.isValid} alignment=${kops.alignment} " +
